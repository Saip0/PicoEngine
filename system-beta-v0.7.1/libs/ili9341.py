...
class Display(object):
    ...
    def draw_image(self, path, x=0, y=0):
        """Desenha imagem RAW RGB565.

        Formato:
            2 bytes = largura (little endian)
            2 bytes = altura  (little endian)
            restante = pixels RGB565
        """
    ...
    def set_rotation(self, rotation):
        ...
        if rotation not in rot_map:
            raise ValueError("Use 0, 90, 180 ou 270")

        madctl, w, h = rot_map[rotation]

        self.write_data_mpy(bytes([madctl]))
    ...
    def draw_text(self, x, y, text, font, color, background=0,
              spacing=1, landscape=False):
        """Draw text using VGA or XGLCD font."""
    ...
    def _get_vga_letter(self, letter, font, color, background=0):
            """Converte uma fonte VGA bitmap para RGB565."""

            # O draw_text() chama esta função com UMA letra
            code = ord(letter)
            ...
            # Bytes necessários para cada linha
            bytes_per_row = (width + 7) // 8

            # Bytes de um caractere
            bytes_per_char = bytes_per_row * height

            # Posição do caractere dentro da fonte
            offset = (code - font.FIRST) * bytes_per_char

            data = font.FONT[offset:offset + bytes_per_char]

            # Cores RGB565
            fg = color.to_bytes(2, "big")
            bg = background.to_bytes(2, "big")
