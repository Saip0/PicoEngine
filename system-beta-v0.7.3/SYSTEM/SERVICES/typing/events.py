from SYSTEM.SERVICES.typing import ui as t_ui
from SYSTEM.INPUT import buttons as but
from SYSTEM.DISPLAY import renderer
class Typ:
    def __init__(self,ui):
        
        self.font_sizes = {
            "8x16": (8,16),
            "16x16": (16,16),
            "16x32": (16, 32),
        }
        #print(t_ui.UI)
        #print(len(t_ui.UI))
        #print(t_ui.UI[0])
        #print(t_ui.UI)
        self.ui = ui
        self.tc = but.Tc_input()
        self.tc_map = self.tc.tc_maps
        self.Map = 0
        self.Pag = 0
        
        t_ui.UI = ui
        txt = False
        if len(t_ui.UI) > 0:
            for elmt in t_ui.UI:
                if elmt["type"] == "text" and elmt["action"] == "Typ":
                    self.txt_ui = elmt
                    self.texto = elmt["text"]
                    self.posit = elmt["layout"]["pos"]
                    self.pos_lst = elmt["layout"]["pos"]
                    self.text_list = elmt["text"]
                    self.lin = elmt["lin"]
                    self.font = elmt["font"]
                    #print(elmt)
                    #print("Linha De Recepção Valor:",self.lin)
                    
                    txt = True
                elif elmt["type"] == "rect":
                    self.surfice = elmt
                    
            for elmt in t_ui.UI:
                if elmt["type"] == "rect" and not txt:
                    self.texto = ""
                    self.pos_lst = []
                    self.text_list = []
                    self.tamn_list = []
                    self.lin = 0
                    #print("Linha De Criaçao Valor:",self.lin)
                    
                    self.text_list.append(self.texto)
                    self.pos_lst.append(elmt["layout"]["pos"])
                    #print(elmt["type"])
                    self.surfice = elmt
                    #self.posit = elmt["layout"]["pos"]
                    self.font = self.surfice["font"]
                    
                    self.txt_ui = (t_ui.text_ui(text=self.text_list,
                                               pos=self.pos_lst,font=self.font,action="Typ",
                                               interact=False,cor="PRETO",back_cor=elmt["cor"],
                                               line=self.lin))
            if not self.txt_ui in t_ui.UI:
                t_ui.UI.append(self.txt_ui)
        
    def escrever(self,tc_state):
        #print(tc_state, tc_state)
        if tc_state not in ["","OK","Map","DEL","RET","Pag",None]:
            
            if self.txt_ui["layout"]["size"][0] < self.surfice["layout"]["pos"][0]+(
                    self.surfice["layout"]["size"][0])*0.75 and self.pos_lst[self.lin][1] < int(self.surfice["layout"]["size"][1]*0.75):
            
                        #self.txt_ui["layout"]["size"][1]*self.lin < (
                    #self.surfice["layout"]["size"][1])):
                    self.text_list[self.lin] += tc_state
    
    #Quebra de Linha
                    #self.font_sizes[self.font]aaaa
        #if self.txt_ui["layout"]["size"][0] > self.surfice["layout"]["pos"][0]+(
         #       self.surfice["layout"]["size"][0])*0.75:
         
        if self.txt_ui["layout"]["size"][0] > +(self.surfice["layout"]["size"][0])*0.75:
            
            self.pos_lst.append([self.surfice["layout"]["pos"][0],
                                 len(self.text_list)*t_ui.font_sizes[self.font][1]+self.surfice["layout"]["pos"][1]])
            self.text_list.append("")
            self.lin += 1
            #print("limite")
            #self.txt_ui["text"] += tc_state
        #print(self.txt_ui["text"])
        #print(self.txt_ui["text"])
            
    def apagar(self,tc_state):
        if tc_state == "DEL":
            
            if len(self.text_list[self.lin]) > 0:
                self.text_list[self.lin] = self.text_list[self.lin][:-1]
                
            elif len(self.text_list[self.lin]) < 1 and self.lin > 0:
                #self.text_list.pop(self.lin)
                antig_lin = self.lin
                self.lin -= 1
                #print(antig_lin)
                #print(self.lin)
                self.text_list[self.lin] = self.text_list[self.lin][:-1]
                self.text_list.pop(antig_lin)
                
            renderer.render([self.surfice])
            renderer.render([self.txt_ui],True)
            #self.render_all()
            
    def render(self):
        renderer.render([{
            "type": "text",
            "text": self.text_list[self.lin],
            "lin" : self.lin,
            "cor" : "PRETO",
            "back_cor" : self.surfice["cor"],
            "font": self.font,
            "layout": {
                "pos": self.pos_lst[self.lin],
                "size": ((len(self.text_list[self.lin]) * 8), 16)
            },
            "action": "Typ",
            "interact": False
        }])
    
    def gerenciar_map(self,tc_state):
        if tc_state == "Map":
            if self.Map < len(self.tc_map)-1:
                self.Map += 1
            else:
                self.Map = 0
            self.Pag = 0
            
        elif tc_state == "Pag":
            if self.Pag < len(self.tc_map[self.Map])-1:
                self.Pag += 1
            else:
                self.Pag = 0
            
            
    def loop(self):
        #print(self.txt_ui)
        #print(self.text_list)
        
        renderer.render([self.txt_ui],True)
        while True:
            #print(self.txt_ui)
            #print(self.txt_ui["layout"]["size"][0])
            #print(self.pos_lst[self.lin][1])
            
            
            tc_state = self.tc.read(Map=self.Map,Pag=self.Pag)
            if tc_state == "RET":
                self.Map = 0
                self.Pag = 0
                #self.render_all()
                for elmt in self.ui:
                    if elmt["action"] == "Typ":
                        self.ui.remove(elmt)
                return self.ui.append(self.txt_ui)
            
            #print(self.ui)
            #print(self.text_list)
            #print(self.lin)
            #print(len(self.text_list))
            #print(self.lin)
            #print(len(self.text_list[self.lin]))
            #print(self.pos_lst[self.lin])
            
            self.tc_map = self.tc.tc_maps
            self.escrever(tc_state)
            self.apagar(tc_state)
            self.gerenciar_map(tc_state)
            
            self.txt_ui = t_ui.text_ui(text=self.text_list,
                                    pos=self.pos_lst,font=self.font,action="Typ",
                                    interact=False,cor="PRETO",back_cor=self.surfice["cor"],
                                       line=self.lin)
            self.render()
            #print("Map:",self.Map)
            #print("Pag:",self.Pag)
            #print(self.lin)
            #print(self.surfice["layout"]["pos"][0]+(self.surfice["layout"]["size"][0]))
            #print(self.txt_ui["layout"]["size"][0])
            #print(self.txt_ui["self.render()
            #renderer.render([self.txt_ui],True )
            #print(self.lin)